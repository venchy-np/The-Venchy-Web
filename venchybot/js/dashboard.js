...
// ── Discord Guild Permission Constants ─────────────────────────
...
window.loadDashboardData = async function () {
    ...
    try {
        ...
        if (!discordResponse.ok) throw new Error("Failed to fetch Discord servers.");
        const allUserGuilds = await discordResponse.json();

        // Filter guilds where user has Administrator or Manage Server
        const adminGuilds = allUserGuilds.filter(g => {
            const perms = BigInt(g.permissions);
            return (perms & PERM_ADMIN) === PERM_ADMIN || (perms & PERM_MANAGE_GUILD) === PERM_MANAGE_GUILD;
        });

        const adminGuildIds = adminGuilds.map(g => g.id);

        // 2. Fetch Guilds configured in Firebase (where bot is installed)
        const guildsSnap = await getDocs(collection(db, "guilds"));
        const botGuilds = [];
        guildsSnap.forEach((doc) => {
            botGuilds.push({ id: doc.id, ...doc.data() });
        });

        // 3. Intersect User's Admin Guilds with Bot's Guilds
        // Or if you want them to be able to SELECT an admin guild even if bot hasn't saved it to FB yet,
        // we can just list administrative guilds. But generally we only want where the bot is.
        // Let's show all guilds they manage, but note if the bot is not configured yet.
        ...
        adminGuilds.forEach(guild => {
            const isBotInGuild = botGuilds.some(bg => bg.id === guild.id);
            const option = document.createElement("option");
            option.value = guild.id;
            
            // Highlight if bot is configured or not
            option.textContent = guild.name + (isBotInGuild ? "" : " (Setup Needed)");
        ...
        if (guildCount === 1) {
            ...
            showToast("You are not an admin of any servers, or could not load.", "info");
        }

    } catch (error) {
        console.error("Error loading dashboard data:", error);
        showToast("Failed to load your servers: " + error.message, "error");
    }
};
...
window.addYouTubeFeed = async function () {
    ...
    if (!channelId || !channelName) return showToast("Channel ID and Name are required", "error");
    if (!channelId.startsWith("UC") || channelId.length < 20) return showToast("Invalid Channel ID — must start with 'UC'", "error");
    ...
    try {
        ...
        await setDoc(doc(db, "guilds", currentGuildId, "youtube_feeds", channelId), {
            ...
            added_by: window.discordUser?.id || "dashboard",
        });
...
window.removeYouTubeFeed = async function (channelId) {
    ...
    try {
        await deleteDoc(doc(window.firebaseDb, "guilds", currentGuildId, "youtube_feeds", channelId));
...
window.addInstagramFeed = async function () {
    ...
    if (!rssUrl || !label) return showToast("RSS URL and Label are required", "error");
    if (!rssUrl.startsWith("http")) return showToast("Invalid URL — must start with http:// or https://", "error");
...
window.removeInstagramFeed = async function (label) {
    ...
    try {
        await deleteDoc(doc(window.firebaseDb, "guilds", currentGuildId, "instagram_feeds", label));
